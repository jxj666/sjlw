...
(function () {
     ...
     * @private
     * @ignore
     * @param form  审辑器所在的form元素
     * @param editor  审辑器实例对象
     */
     ...
     * @desc 创建一个跟审辑器实例
     * - ***container*** 审辑器容器对象
     * - ***iframe*** 审辑区域所在的iframe对象
     * - ***window*** 审辑区域所在的window
     * - ***document*** 审辑区域所在的document对象
     * - ***body*** 审辑区域所在的body对象
     * - ***selection*** 审辑区域的选区对象
     */
    ...
    Editor.prototype = {
        /**
         * 当审辑器ready后执行传入的fn,如果审辑器已经完成ready，就马上执行fn，fn的中的this是审辑器实例。
         * 大部分的实例接口都需要放在该方法内部执行，否则在IE下可能会报错。
         * @name ready
         * @grammar editor.ready(fn) fn是当审辑器渲染好后执行的function
         * @example
         ...
         * @private
         * @ignore
         * @param {Element} doc 审辑器Iframe中的文档对象
         */
        ...
        /**
         * 同步审辑器的数据，为提交数据做准备，主要用于你是手动提交的情况
         * @name sync
         * @grammar editor.sync(); //从审辑器的容器向上查找，如果找到就同步数据
         * @grammar editor.sync(formID); //formID制定一个要同步数据的form的id,审辑器的数据会同步到你指定form下
         * @desc
         ...
         * 获取审辑器内容
         * @name getContent
         * @grammar editor.getContent()  => String //若审辑器中只包含字符"&lt;p&gt;&lt;br /&gt;&lt;/p/&gt;"会返回空。
         * @grammar editor.getContent(fn)  => String
         * @example
         * getContent默认是会现调用hasContents来判断审辑器是否为空，如果是，就直接返回空字符串
         * 你也可以传入一个fn来接替hasContents的工作，定制判断的规则
         * editor.getContent(function(){
         *     return false //审辑器没有内容 ，getContent直接返回空
         * })
         */
        ...
        /**
         * 将html设置到审辑器中, 如果是用于初始化时给审辑器赋初值，则必须放在ready方法内部执行
         * @name setContent
        ...
        /**
         * 让审辑器获得焦点，toEnd确定focus位置
         * @name focus
         * @grammar editor.focus([toEnd])   //默认focus到审辑器头部，toEnd为true时focus到内容尾部
         */
...
//            }
        ...
        /**
         * 根据传入的command命令，查选审辑器当前的选区，返回命令的状态
        ...
        /**
         * 根据传入的command命令，查选审辑器当前的选区，根据命令返回相关的值
        ...
        /**
         * 检查审辑区域中是否有内容，若包含tags中的节点类型，直接返回true
         * @name  hasContents
         * @desc
         ...
         * @example
         * editor.hasContents(['span']) //如果审辑器里有这些，不认为是空
         */
