 ...
 * 主要有两种修改方案，一种是取消此处注释，然后修改成对应参数；另一种是在实例化审辑器时传入对应参数。
 * 当升级审辑器时，可直接使用旧版配置文件替换新版配置文件,不用担心旧版配置文件中因缺少新功能所需的参数而导致脚本报错。
 **************************提示********************************/
...
(function () {
    /**
     * 审辑器资源文件根路径。它所表示的含义是：以审辑器实例化页面为当前路径，指向审辑器资源文件（即dialog等文件夹）的路径。
     * 鉴于很多同学在使用审辑器的时候出现的种种路径问题，此处强烈建议大家使用"相对于网站根目录的相对路径"进行配置。
     * "相对于网站根目录的相对路径"也就是以斜杠开头的形如"/myProject/umeditor/"这样的路径。
     * 如果站点中有多个不在同一层级的页面需要实例化审辑器，且引用了同一UEditor的时候，此处的URL可能不适用于每个页面的审辑器。
     * 因此，UEditor提供了针对不同页面的审辑器可单独配置的根路径，具体来说，在需要实例化审辑器的页面最顶部写上如下代码即可。当然，需要令此处的URL等于对应的配置。
    ...
    window.UMEDITOR_CONFIG = {
        ...
        //工具栏上的所有的功能按钮和下拉框，可以在new审辑器的实例时选择自己需要的从新定义
        ,toolbar:[
        ...
        //针对getAllHtml方法，会在对应的head标签中增加该审码设置。
        //,charset:"utf-8"

        //常用配置项目
        //,isShow : true    //默认显示审辑器

        //,initialContent:'欢迎使用UMEDITOR!'    //初始化审辑器的内容,也可以通过textarea/script给值，看官网例子

        //,initialFrameWidth:500 //初始化审辑器宽度,默认500
        //,initialFrameHeight:500  //初始化审辑器高度,默认500

        //,autoClearinitialContent:true //是否自动清除审辑器初始内容，注意：如果focus属性设置为true,这个也为真，那么审辑器一上来就会触发导致初始化的内容看不到了

        //,textarea:'editorValue' // 提交表单时，服务器获取审辑器提交内容的所用的参数，多实例时可以给容器name属性，会将name给定的值最为每个实例的键值，不用每次实例化的时候都设置这个值

        //,focus:false //初始化时，是否让审辑器获得焦点true或false

        //,autoClearEmptyNode : true //getContent时，是否删除空的inlineElement节点（包括嵌套的情况）

        //,fullscreen : false //是否开启初始化时即全屏，默认关闭

        //,readonly : false //审辑器初始化结束后,审辑区域是否是只读的，默认是false

        //,zIndex : 900     //审辑器层级的基数,默认是900

        //如果自定义，最好给p标签如下的行高，要不输入中文时，会有跳动感
        //注意这里添加的样式，最好放在.edui-editor-body .edui-body-container这两个的下边，防止跟页面上css冲突
        //,initialStyle:'.edui-editor-body .edui-body-container p{line-height:1em}'

        //,autoSyncData:true //自动同步审辑器要提交的数据
